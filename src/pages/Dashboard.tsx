import { useClients, useInterventions, useDevis, useMachines } from "@/hooks/use-data";
import { Card } from "@/components/ui/card";
import { StatusBadge } from "@/components/StatusBadge";
import { Wrench, Users, FileText, Clock, TrendingUp, AlertTriangle, Loader2, ArrowRight } from "lucide-react";
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
        <Card className="stat-card">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-primary/15 flex items-center justify-center"><Users className="w-5 h-5 text-primary" /></div>
            <div><p className="text-2xl font-bold">{clients.length}</p><p className="text-xs text-muted-foreground">Clients</p></div>
          </div>
        </Card>
        <Card className="stat-card">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-warning/15 flex items-center justify-center"><Wrench className="w-5 h-5 text-warning" /></div>
            <div><p className="text-2xl font-bold">{interventionsEnCours}</p><p className="text-xs text-muted-foreground">En cours</p></div>
          </div>
        </Card>
        <Card className="stat-card">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-info/15 flex items-center justify-center"><Clock className="w-5 h-5 text-info" /></div>
            <div><p className="text-2xl font-bold">{interventionsPlanifiees}</p><p className="text-xs text-muted-foreground">Planifiées</p></div>
          </div>
        </Card>
        <Card className="stat-card">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-success/15 flex items-center justify-center"><TrendingUp className="w-5 h-5 text-success" /></div>
        ...
        <Card className="stat-card">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-warning/15 flex items-center justify-center"><FileText className="w-5 h-5 text-warning" /></div>
        ...
        <Card className="stat-card">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-destructive/15 flex items-center justify-center"><AlertTriangle className="w-5 h-5 text-destructive" /></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-6 rounded-2xl">
          ...
          <div className="space-y-1">
            {recentInterventions.map(inter => {
              ...
              return (
                <div key={inter.id} className="flex items-center justify-between py-2.5 border-b border-border/40 last:border-0">
          ...
          <Link to="/interventions" className="text-sm text-primary font-medium mt-4 inline-flex items-center gap-1 hover:underline">
        ...
        <Card className="p-6 rounded-2xl">
          ...
          <Link to="/devis" className="text-sm text-primary font-medium mt-4 inline-flex items-center gap-1 hover:underline">
