...
import { MapPin, Clock, Route, Navigation, Loader2 } from "lucide-react";
...
export default function RoutePlanning() {
  ...
  // Set default tech once loaded
  if (!selectedTech && technicians.length > 0) {
    setSelectedTech(technicians[0].id);
  }

  const techInterventionsForDate = interventions
    .filter(i => i.technician_id === selectedTech && i.status !== 'terminee' && i.date === selectedDate);
  ...
  const totalTravel = interventionsWithClients.reduce((s, i) => s + (i.travel_time || 0), 0);
  const totalDuration = interventionsWithClients.reduce((s, i) => s + (i.duration || 0) + (i.travel_time || 0), 0);
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="h-10 rounded-md border border-input bg-background px-3 text-sm" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <Card className="stat-card"><div className="flex items-center gap-2"><Navigation className="w-5 h-5 text-primary" /><div><p className="text-lg font-bold">{interventionsWithClients.length}</p><p className="text-xs text-muted-foreground">Interventions</p></div></div></Card>
        <Card className="stat-card"><div className="flex items-center gap-2"><Clock className="w-5 h-5 text-warning" /><div><p className="text-lg font-bold">{totalTravel} min</p><p className="text-xs text-muted-foreground">Temps de trajet</p></div></div></Card>
        <Card className="stat-card"><div className="flex items-center gap-2"><Clock className="w-5 h-5 text-info" /><div><p className="text-lg font-bold">{Math.floor(totalDuration / 60)}h{String(totalDuration % 60).padStart(2, '0')}</p><p className="text-xs text-muted-foreground">Durée totale</p></div></div></Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-accent/5 h-80 lg:h-full min-h-[320px] flex items-center justify-center relative">
            ...
            <div className="relative w-full h-full p-6">
              ...
              {interventionsWithClients.map((inter, idx) => {
                ...
                return (
                  <div key={inter.id} className="absolute flex flex-col items-center" style={{ left: `${x}%`, top: `${y}%`, transform: 'translate(-50%, -50%)' }}>
              ...
              {optimized && interventionsWithClients.length > 1 && (
                <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 0 }}>
                  {interventionsWithClients.slice(0, -1).map((inter, idx) => {
                    ...
                    return <line key={idx} x1={`${x1}%`} y1={`${y1}%`} x2={`${x2}%`} y2={`${y2}%`} stroke="hsl(24, 95%, 53%)" strokeWidth="2" strokeDasharray="6,4" />;
                  })}
                </svg>
              )}
              {interventionsWithClients.length === 0 && (
                <div className="absolute inset-0 flex items-center justify-center"><p className="text-sm text-muted-foreground">Aucune intervention pour cette date</p></div>
              )}
            </div>
          </div>
        </Card>
        ...
        <div className="space-y-3">
          ...
          {interventionsWithClients.length === 0 ? (
            <Card className="p-6 text-center text-muted-foreground text-sm">Aucune intervention planifiée pour cette date</Card>
          ) : (
            interventionsWithClients.map((inter, idx) => (
